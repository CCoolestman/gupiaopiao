...
public class AllinfoService extends BaseService<ListBean<AllinfoBean>> {
    ...
    protected Object getResponseBodyBean(HttpServletRequest req) {
        ...
        List<AllinfoBean> allinfo = allinfoDao.getAllinfo(proportion, proportionOrder, quoteChange, quoteChangeOrder, page, pageSize);
        ListBean<AllinfoBean> bean = new ListBean<>();
        bean.setData(allinfo);
        bean.setPage(page);
        bean.setPageSize(pageSize);
        bean.setTotal(allinfoDao.getTotal(proportion, proportionOrder, quoteChange, quoteChangeOrder));
        return bean;
    }
}
